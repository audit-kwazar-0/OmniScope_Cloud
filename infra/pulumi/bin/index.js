...
exports.acrLoginServer = exports.acrName = exports.privateEndpointsSubnetId = exports.vnetId = exports.aksFqdn = exports.aksName = exports.eventHubId = exports.grafanaUrl = exports.azureMonitorWorkspaceId = exports.actionGroupId = exports.applicationInsightsId = exports.logAnalyticsWorkspaceId = exports.resourceGroupName = void 0;
...
const eventhub = __importStar(require("@pulumi/azure-native/eventhub"));
const insights = __importStar(require("@pulumi/azure-native/insights"));
const insightsv20250101preview = __importStar(require("@pulumi/azure-native/insights/v20250101preview/scheduledQueryRule"));
const managedidentity = __importStar(require("@pulumi/azure-native/managedidentity"));
...
const insightEnums = __importStar(require("@pulumi/azure-native/types/enums/insights"));
const dashboardEnums = __importStar(require("@pulumi/azure-native/types/enums/dashboard"));
const csEnums = __importStar(require("@pulumi/azure-native/types/enums/containerservice"));
const insightRuleEnums = __importStar(require("@pulumi/azure-native/types/enums/insights/v20250101preview"));
...
function subscriptionRoleDefinition(roleId) {
...
const actionGroup = new insights.ActionGroup("actionGroup", {
    ...
    webhookReceivers: teamsWebhookUri.length > 0
        ? [
            {
                name: "teams-webhook",
...
if (deployLogExport) {
    ...
    new operationalinsights.DataExport("law-export-alllogs", {
        resourceGroupName: rg.name,
...
const cpuHighQuery = 'let avgCpu = toscalar(InsightsMetrics | where TimeGenerated > ago(5m) | where Namespace == "container.azm.ms/cpu" and Name == "cpuUsageNanoCores" | summarize avg(Val)); print cpuBreach = iif(isnull(avgCpu), 0.0, iif(avgCpu > 80.0, 1.0, 0.0))';
...
const normalizedAcrOverride = acrNameOverride.length > 0
    ? acrNameOverride.replace(/-/g, "").toLowerCase().slice(0, 50)
    : "";
const resolvedAcrName = acrNameOverride.length > 0
...
if (deployAks) {
    ...
    const aksAddonProfiles = {
        ...
        ...(enableAzurePolicyAddon ? { azurepolicy: { enabled: true } } : {}),
...
if (deployAks && deployAcr && acr && aks) {
    const kubeletObjectId = aks.identityProfile.apply((p) => p?.kubeletidentity?.objectId ?? "");
...
exports.azureMonitorWorkspaceId = azureMonitorWorkspace?.id ?? pulumi.output("");
exports.grafanaUrl = managedGrafana?.properties.apply((p) => p.endpoint ?? "") ?? pulumi.output("");
exports.eventHubId = ehLogs?.id ?? pulumi.output("");
exports.aksName = aks?.name ?? pulumi.output("");
exports.aksFqdn = aks?.fqdn ?? pulumi.output("");
exports.vnetId = vnet?.id ?? pulumi.output("");
exports.privateEndpointsSubnetId = vnet !== undefined
    ? pulumi.interpolate `${vnet.id}/subnets/private-endpoints`
    : pulumi.output("");
exports.acrName = acr?.name ?? pulumi.output("");
exports.acrLoginServer = acr?.loginServer ?? pulumi.output("");
